"""Authenticates Spotify, gets song information

Checks if Spotify is already authenticated using pickle token.
 If so, continues with the Spotify build. If not, uses client secret to authenticate, update pickle, then continues.
 Inputs the dictionary that includes the song titles for each video. For each song title, matches Spotify's top
 song with the same name and returns all artists. Updates dictionary entries from [Video, [songs]] to [Video, [song, artists]].
...
def spotify_artist_get(song_dict: dict) -> dict:
