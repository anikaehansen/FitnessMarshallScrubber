...
def process_playlist_and_filter_comments(playlist_id, youtube):
    # Step 1: Get videos from the playlist
    videos_in_playlist, video_urls_in_playlist = get_playlist_videos(playlist_id, youtube)
...
def isolate_songs_from_comments(filtered_comments):
    song_dict = {}
    pattern = r'">.*?<\/a>\s*([^<]+)'
    ...
    for comment in filtered_comments:
        ...
        for idx, song in enumerate(song_list):
            ...
            if 'Stretch' in song:
                ...
                if match:
                    ...
                    song_list[idx] = match.group(1)  # Update the song name to only the content inside parentheses
        ...
        filtered_song_list = [
            ...
            if song.strip() and not any(exclude_word.lower() in song.lower() for exclude_word in exclude_words) and song != 'Open ' and song != 'Open'
        ]
