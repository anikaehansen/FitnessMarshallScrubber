...
class YT_Posts():
    ...
    def cleanUpPostResults(self, post_dict: dict) -> dict:
        ...
        result_dict["postParams"] = post_dict["publishedTimeText"]["runs"][0]["navigationEndpoint"]["browseEndpoint"][
            "params"]
            ...
            if "pollRenderer" in post_dict["backstageAttachment"].keys():
                ...
                for each_choice in post_dict["backstageAttachment"]["pollRenderer"]["choices"]:
                    ...
                    if "numVotes" in each_choice:
                        ...
                        choice_dict["votePercentage"] = each_choice["votePercentageIfNotSelected"][
    ...
    def cleanUpCommentResults(self, comment_dict: dict, reply: bool) -> dict:
        ...
        if not reply and "replies" in comment_dict["commentThreadRenderer"].keys():
            result_dict['replyToken'] = \
            comment_dict["commentThreadRenderer"]["replies"]["commentRepliesRenderer"]["contents"][0][
                'continuationItemRenderer']['continuationEndpoint']['continuationCommand']['token']
            result_dict['replyCount'] = comment_dict["commentThreadRenderer"]["comment"]["commentRenderer"][
                "replyCount"]
    ...
    def fetchPosts(self, channel_id: str, limit: int = 10) -> dict:
        ...
        community_posts = \
        r['contents']['twoColumnBrowseResultsRenderer']['tabs'][3]['tabRenderer']['content']['sectionListRenderer'][
        ...
        if limit > 10:
            for _ in range(10, limit, 10):
                if 'continuationItemRenderer' in community_posts[-1].keys():
                    ...
                    post_data = copy.deepcopy(POST_DATA)
                    post_data.update(json.loads(CONTINUATION_PARAM.format(
                        token=continuation['continuationItemRenderer']['continuationEndpoint']['continuationCommand'][
                            'token'])))
                    r = requests.post(BASE_URL, headers=self.headers, data=json.dumps(post_data)).json()
                    community_posts.extend(
                        r['onResponseReceivedEndpoints'][0]['appendContinuationItemsAction']['continuationItems'])
    ...
    def fetchPost(self, channel_id: str, post_id: str) -> dict:
        ...
        r = requests.post(BASE_URL, headers=self.headers, data=json.dumps(post_data)).json()
        result = \
        r['contents']['twoColumnBrowseResultsRenderer']['tabs'][0]['tabRenderer']['content']['sectionListRenderer'][
            'contents']
        ...
        result[1]["itemSectionRenderer"]["contents"][0]['continuationItemRenderer']['continuationEndpoint'][
    ...
    def fetchComment(self, token: str, limit: int = 20, reply: bool = False) -> dict:
        ...
        if not reply:
            commentCount = \
            r['onResponseReceivedEndpoints'][0]['reloadContinuationItemsCommand']['continuationItems'][0][
                "commentsHeaderRenderer"]["commentsCount"]["runs"][0]["text"]
